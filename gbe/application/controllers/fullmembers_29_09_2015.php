...
class Fullmembers extends CI_Controller {
    ...
    public function index() {
		...
		$viewData["cityList"] = $this->gatewaymodel->getCityListByCountryId($viewData["userInfo"][0]['country']);
	...
	public function calendarEvent(){
		...
		$retData = array();
		$retData['current'] = $this->getMonthCalender($year,$month);
		$retData['qtrly'] = $this->getQuaterlyCalender($year,$month);
		$retData['yearly'] = $this->getYearlyCalender($year);	
		//print_r($retData['qtrly']);exit;
	...
	public function getMonthCalender($year,$month){
		...
		$dbData = $this->fullmembers_model->getEventDetailsMonthWise($year,$month);
	...
	public function getQuaterlyCalender(){
		$year = date("Y");
		$month = date("m");
		if($month == 12){
			$year1 = $year;
			$month1 = str_pad(($month - 1), 2, "0", STR_PAD_LEFT);
			$dbData = $this->fullmembers_model->getEventDetailsMonthWise($year1,$month1);
			$data1 = $this->manipulateEventData($dbData,$year1,$month1,'q');
			unset($dbData);
			$year2 = $year;
			$month2 = str_pad($month, 2, "0", STR_PAD_LEFT);
			$dbData = $this->fullmembers_model->getEventDetailsMonthWise($year2,$month2);
			$data2 = $this->manipulateEventData($dbData,$year2,$month2,'q');
			unset($dbData);
			$year3 = $year + 1;
			$month3 = str_pad(1, 2, "0", STR_PAD_LEFT);;
			$dbData = $this->fullmembers_model->getEventDetailsMonthWise($year3,$month3);
			...
			unset($dbData);
			$year3 = $year;
			$month3 = str_pad(($month + 1), 2, "0", STR_PAD_LEFT);
			$dbData = $this->fullmembers_model->getEventDetailsMonthWise($year3,$month3);
			$data3 = $this->manipulateEventData($dbData,$year3,$month3,'q');
			unset($dbData);
		}else{
			$month1 = str_pad(($month - 1), 2, "0", STR_PAD_LEFT);
			$dbData = $this->fullmembers_model->getEventDetailsMonthWise($year,$month1);
			$data1 = $this->manipulateEventData($dbData,$year,$month1,'q');
			unset($dbData);
			$month2 = str_pad($month, 2, "0", STR_PAD_LEFT);
			$dbData = $this->fullmembers_model->getEventDetailsMonthWise($year,$month2);
			$data2 = $this->manipulateEventData($dbData,$year,$month2,'q');
			unset($dbData);
			$month3 = str_pad(($month + 1), 2, "0", STR_PAD_LEFT);
			$dbData = $this->fullmembers_model->getEventDetailsMonthWise($year,$month3);
	...
	public function getYearlyCalender(){
		$year = date("Y");
		$month = date("m");
		$config = $this->setCalendarTemplate();
		$config['show_next_prev'] = FALSE;
		...
		for($i = 1;$i<13;$i++){
			...
			$dbData = $this->fullmembers_model->getEventDetailsMonthWise($year,$month);
			$data = $this->manipulateEventData($dbData,$year,$month,'l');
			$data = array();
	...
	public function setCalendarTemplate(){
		$prefs['template'] = '
			...
			{heading_previous_cell}<th><a class="pvnx" href="{previous_url}">&lt;</a></th>{/heading_previous_cell}
			{heading_title_cell}<th colspan="{colspan}">{heading}</th>{/heading_title_cell}
			{heading_next_cell}<th><a class="pvnx" href="{next_url}">&gt;</a></th>{/heading_next_cell}
	...
	public function manipulateEventData($data = array(),$year,$month,$typeCal='c'){
		...
		if(is_array($data) && count($data) > 0){
			foreach($data as $v){
				$dumyData[$v->yyyy][$v->mm][$v->dd][] = array('id'=>$v->id,'name'=>$v->name,'location'=>$v->location,'contact'=>$v->contact_number,'start_date'=>$v->start_date,'end_date'=>$v->end_date,'image_path'=>$v->image_path);
			}
		}
				...
				if(is_array($vv) && count($vv) > 0){
					...
					for($i=0;$i<2;$i++){
						if(isset($vv[$i]['id']) && $vv[$i]['name'] != '' && $vv[$i]['id'] > 0){
							$eventName .= '<span class="eventsNameClass" id="'.$typeCal.'_'.$vv[$i]['id'].'">
											'.$vv[$i]['name'].'
											<span class="toltip_event" id="toltip_event_'.$typeCal.'_'.$vv[$i]['id'].'" name="" style="display:none;">
												<span class="close" name="'.$typeCal.'_'.$vv[$i]['id'].'">x</span>
					...
					if(count($vv) > 2){
						$eventName .= '<span>More <strong>'.(count($vv)-2).'</strong></span>';
					}
