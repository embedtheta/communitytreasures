...
class Admin extends CI_Controller {
    ...
    public function dashboard() {
        $view_data = array();
		$view_data['navMenu'] = $this->data_model->SubMenusArticleMng();// added by SB on 28/04/2015
		//print_r($view_data['navMenu']);
    ...
    public function addCreative() {
	 ...
	 public function addArticle() {
		...
		$viewData["menuTypes"] = $this->data_model->SubMenusArticleMng();
        ...
        if (trim($this->input->post('submit'))) {
            ...
            if ($this->form_validation->run() != FALSE) {
                $lastInsertId = $this->data_model->insertArticle($viewData);
                if ($lastInsertId > 0) {
...
// Added by SB on 29/04/2015
        ...
        if (trim($this->input->post('submit'))) {
            ...
            if ($this->form_validation->run() != FALSE) {
                ...
                if ($updateStatus) {
                   ...
                   redirect('admin/updatedArticlesData/'.$viewData["menuID"]. '/'.$viewData["articleID"]);
						
					/*$updatedata['navMenu'] = $this->data_model->getMenusList();// added by SB on 28/04/2015
					$updatedata['selectedMenuId'] = $viewData["menuID"];
					$updatedata['categoryName'] = $this->data_model->getMenuName($viewData["menuID"]);// added by SB on 28/04/2015
					$updatedata['infoArticle'] = $this->data_model->getupdateArticleList($viewData["articleID"]);
					//print_r($updatedata['infoArticle']);exit;
					$this->load->view('adminTemplates/articles/viewArticles', $updatedata);	*/	
 ...
 // added by Sb on 29/04/2015
	...
	public function viewArticles() {
		...
		$page = 0;
		$uri_segment = 4;
		$viewData["selectedArts"]="";
        if($this->uri->segment(4) >0 && $this->uri->segment(4) != ""){
            $page = $this->uri->segment(4);
        }
		
		if ($this->input->post('go')!="") {
			      $viewData["selectedArts"] = trim($this->input->post('search'));
					
		}

		$viewData['navMenu'] = $this->data_model->SubMenusArticleMng();// added by SB on 28/04/2015
		$viewData['selectedMenuId'] = $menuId;
		$viewData['categoryName'] = $this->data_model->getMenuName($menuId);// added by SB on 28/04/2015
		$viewData["infoArticle"] = $this->data_model->getMenuContentDetails($menuId,$per_page,$page,$viewData["selectedArts"]);
		
		//print_r($viewData["infoArticle"]);exit;
        $viewData["pagination"] = $this->commonPagination($base_url,$total_rows,$per_page,$uri_segment);
        $this->load->view('adminTemplates/articles/viewArticles', $viewData);
    }
	public function updatedArticlesData() {
			$updatedata = array();
            $updatedata["submissionReport"] = "One item edited sucessfuly";
               
		$menuId = trim($this->uri->segment(3));	
		$articleID = trim($this->uri->segment(4));
		if($menuId>0)
		{
			$base_url = base_url()."admin/viewArticles/".$menuId."/";
		}
		...
		if ($this->input->post('go')!="") {
			      $updatedata["selectedArts"] = trim($this->input->post('search'));
					
		}

		$updatedata['navMenu'] = $this->data_model->SubMenusArticleMng();// added by SB on 28/04/2015
		$updatedata['selectedMenuId'] = $menuId;
		$updatedata['categoryName'] = $this->data_model->getMenuName($menuId);// added by SB on 28/04/2015
		$updatedata['infoArticle'] = $this->data_model->getupdateArticleList($menuId,$articleID);
...
public function unlinkOldFile($channelID,$imageFieldname){
	...
	public function notificationSend(){
		...
		if($userGroup!="" && $userLevel!=""  && $notTitle!="" && $notMessage!=""){
			...
			$data['forWebsite']		=   $this->forWebsite;
	...
	//new added us 6/1/2016------------------------------------------------------------------------------------------
	 public function viewSourceZone() {
        $viewData = array();
        if ($_REQUEST["action"] == "add") {
            $viewData["submissionReport"] = "One item added sucessfuly";
        }
		...
		if($menuId>0)
		{
			$base_url = base_url()."admin/viewSourceZone/".$menuId."/";
		}
		else{
			$base_url = base_url()."admin/viewSourceZone/0/";
		}
		$total_rows = $this->data_model->getTotalSourceContentDetail();
		$per_page = $this->dataPerPage;
		$page = 0;
		$uri_segment = 4;
		...
		//$viewData['selectedMenuId'] = $menuId;
		//$viewData['categoryName'] = $this->data_model->getMenuName($menuId);// added by SB on 28/04/2015
		$viewData["infoArticle"] = $this->data_model->getSourceContentDetails($per_page,$page);
		//print_r($viewData["infoArticle"]);exit;
        $viewData["pagination"] = $this->commonPagination($base_url,$total_rows,$per_page,$uri_segment);
        $this->load->view('adminTemplates/ct_source/viewSource', $viewData);
    }

	public function addCTSourceZone() {
        $viewData = array();
		$viewData['selectedMenuId'] = trim($this->uri->segment(3));// added by SB on 05/08/2015
		//$viewData["menuTypes"] = $this->data_model->getMenusList();
		$viewData["articleTypes"] = $this->data_model->getSourceList();// menu id passed Added by Sb on 05/08/2015
        if (trim($this->input->post('submit'))) 
		{
            $this->load->library('form_validation');
            $this->form_validation->set_rules('articleDesc', 'Description ', 'trim|required');
            ...
            $viewData["articleStatus"] = trim($this->input->post('articleStatus'));
            if ($this->form_validation->run() != FALSE) {
                $lastInsertId = $this->data_model->CT_insertArticleDetails($viewData);
                if ($lastInsertId > 0) {
                    echo "<script type=\"text/javascript\">window.location=\"" . base_url() . "admin/viewSourceZone/".$viewData['selectedMenuId']."?action=add\";</script>";
                } else {
                    $viewData['errorMsg'] = 'Your data has not been inserted.';
                }
            } else {
	    ...
	    $this->load->view('adminTemplates/ct_source/addSource', $viewData);
    }
	public function editSourceZone($articleID = 0) {
        $viewData = array();
		$viewData["articleTypes"] = $this->data_model->getSourceList();
        if (trim($this->input->post('submit'))) {
            $this->load->library('form_validation');
            $this->form_validation->set_rules('articleDesc', 'Description ', 'trim|required');
            if (!empty($_FILES['image']['name'])) {
            ...
            $viewData["articleDesc"] = trim($this->input->post('articleDesc'));
            $viewData["articleStatus"] = trim($this->input->post('articleStatus'));
			$viewData["SourceID"] = $articleID;
			//print_r($viewData);exit;
            if ($this->form_validation->run() != FALSE) {
                $updateStatus = $this->data_model->updateSourceData($viewData);
                if ($updateStatus) {
                    //redirect(base_url().'admin/viewCreative/?action=edit', 'refresh');
                    echo "<script type=\"text/javascript\">window.location=\"" . base_url() . "admin/viewSourceZone/".$articleID."?action=edit\";</script>";
                }
            } else {
                $viewData['errorMsg'] = 'Please check the error(s) below.';
            }
        }
        $viewData["infoArticle"] = $this->data_model->getIndividualSourceleData($articleID);
        if (empty($viewData["infoArticle"])) {
            $viewData["errorMsg"] = "Invalid Data Request";
        }
		//print_r($viewData["infoArticle"]);exit;
		//$viewData['navMenu'] = $this->data_model->getMenusList();// added by SB on 28/04/2015
        $this->load->view('adminTemplates/ct_source/editSource', $viewData);
    }
	public function deleteSource($sourceID = 0,$mID) {
        if ($sourceID > 0) {
            $delRequst = $this->data_model->deleteSource($sourceID);
            if ($delRequst) {
                redirect(base_url() . 'admin/viewSourceZone/'.$mID.'?action=delSuccess', 'refresh');
            } else {
                redirect(base_url() . 'admin/viewSourceZone/'.$mID.'?action=delError', 'refresh');
            }
        } else {
            redirect(base_url() . 'admin/viewSourceZone/'.$mID.'?action=delError', 'refresh');
        }
	...
	// footer banner for CT  added by SB 25/02/2016
	
	    public function viewCTfooterBanner() {
        $viewData = array();
        if ($_REQUEST["action"] == "add") {
            $viewData["submissionReport"] = "One item added sucessfuly";
        }
        ...
        /*         * ************************************************ */
        $viewData["infoFooterBanner"] = $this->data_model->getCtFooterBanner();
		$viewData['navMenu'] = $this->data_model->getMenusList();// added by SB on 28/04/2015
        $this->load->view('adminTemplates/footerBanner/viewCTbanner', $viewData);
    }

    public function addCTfooterBanner() {
        $viewData = array();
        if (trim($this->input->post('submit'))) {
            $this->load->library('form_validation');
            ...
            $viewData["bannerTitle"] = trim($this->input->post('bannerTitle'));
            $viewData["bannerDesc"] = trim($this->input->post('bannerDesc'));
            $viewData["bannerImg"] = $_FILES['image']['name'];
            $viewData["bannerStatus"] = trim($this->input->post('bannerStatus'));

            if ($this->form_validation->run() != FALSE) {
                $lastInsertId = $this->data_model->insertFootBanner($viewData);
                if ($lastInsertId > 0) {
                    //redirect(base_url().'admin/viewArticleZone/?action=add', 'refresh');
                    echo "<script type=\"text/javascript\">window.location=\"" . base_url() . "admin/viewCTfooterBanner/?action=add\";</script>";
                } else {
                    $viewData['errorMsg'] = 'Your data has not been inserted.';
                }
            } else {
        ...
        $this->load->view('adminTemplates/footerBanner/addCTfooterBanner', $viewData);
    }

    public function editCTfooterBanner($footBannID = 0) {
        $viewData = array();

        if (trim($this->input->post('submit'))) {
            $this->load->library('form_validation');
            $this->form_validation->set_rules('footBannID', 'banner ID ', 'trim|required');
            $this->form_validation->set_rules('bannerTitle', 'Title ', 'trim|required');
            ...
            $viewData["footerBanID"] = trim($this->input->post('footBannID'));
			$viewData["bannerTitle"] = trim($this->input->post('bannerTitle'));
            $viewData["bannerDesc"] = trim($this->input->post('bannerDesc'));
            $viewData["bannerStatus"] = trim($this->input->post('bannerStatus'));
            if ($this->form_validation->run() != FALSE) {
                $updateStatus = $this->data_model->updateFootBanner($viewData);
                if ($updateStatus) {
                    //redirect(base_url().'admin/viewArticleZone/?action=edit', 'refresh');
                    echo "<script type=\"text/javascript\">window.location=\"" . base_url() . "admin/viewCTfooterBanner/?action=edit\";</script>";
                }
            } else {
                $viewData['errorMsg'] = 'Please check the error(s) below.';
            }
        }
        $viewData["infoFootBanner"] = $this->data_model->getIndividualFootBanner($footBannID);
        if (empty($viewData["infoFootBanner"])) {
            $viewData["errorMsg"] = "Invalid Data Request";
        }
        $this->load->view('adminTemplates/footerBanner/editCTfooterBanner', $viewData);
    }
    public function deleteCTfooterBanner($footBannID = 0) {
        if ($footBannID > 0) {
            $delRequst = $this->data_model->deleteCTfotterBanner($footBannID);
            if ($delRequst) {
                redirect(base_url() . 'admin/viewCTfooterBanner/?action=delSuccess', 'refresh');
            } else {
                redirect(base_url() . 'admin/viewCTfooterBanner/?action=delError', 'refresh');
            }
        } else {
            redirect(base_url() . 'admin/viewCTfooterBanner/?action=delError', 'refresh');
        }
