	...
	public function selectParentVolunteer($city){
		...
		// check level 1 volunteer  having child count less than 12 take 1
			...
			if(count($minUserChildCount)>0){
				$volData['selectedParentId'] =$minUserChildCount[0]['user_id'];
				$volData['volLevel']= 2;
			...
			// if all level 1 vol having child 12 each then check for level 2 vol
				...
				if($levelTwoVol>0){
					...
					if(count($minUserChildCount2)>0){
						$volData['selectedParentId'] =$minUserChildCount2[0]['user_id'];
						$volData['volLevel']= 3;
						...
						// if all level 2 vol having child 12 each then check for level 3 vol
							...
							if($levelThreeVol>0){
								...
								$volData['selectedParentId'] =$minUserChildCount3[0]['user_id'];
								$volData['volLevel']= 4;
