...
class Fullmembers_model extends CI_Model {
	...
	public function getEventSearchBredCamp($data = array()){
		$country_id = $data['country'];
		$city_id = $data['city'];
		$zip_code_id = $data['zip_code'];
		if($country_id != '' && $country_id > 0){
			...
			if($zip_code_id != '' && $zip_code_id > 0){
				$sql .= ' ,zip_code.id           zip_code_id,
					  	zip_code.zip_code     zip_code_name';
			}
			$sql .= ' FROM country ';
			if($city_id != '' && $city_id > 0){
				$sql .= ' RIGHT JOIN city
			...
			if($zip_code_id != '' && $zip_code_id > 0){
				$sql .= ' RIGHT JOIN zip_code
					    	ON zip_code.city_id = city.id';
			}
			...
			if($zip_code_id != '' && $zip_code_id > 0){
				$sql .= ' AND zip_code.id = '.$zip_code_id;
			}
			$query = $this->db->query($sql);
			return $query->result();
		}else{
