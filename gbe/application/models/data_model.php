...
class data_model extends CI_model{
   ...
   function getIndividualArticleData($articleID){
	   ...
	   if($articleID !=""){
		   $this->db2->select('productmenucontent.*');
	       $this->db2->from('productmenucontent');
	       $this->db2->where('productmenucontent.id',$articleID);
	       $query=$this->db2->get();
	       return $query->result_array(); 
	   }
   }
   ...
   function updateArticleData($data=array()){
	  // echo $data['articleID']);exit;
	   $CI = &get_instance();
		//setting the second parameter to TRUE (Boolean) the function will return the database object.
		$this->db2 = $CI->load->database('ADMINDB', TRUE);
   		if(!empty($data)){
			if(isset($data['articleImg'])&&trim($data['articleImg']) != ""){
			   $this->db2->set('productmenucontent.image',trim($data['articleImg']));
		    }
		    if(trim($data['articleTitle']) != ""){
			   $this->db2->set('productmenucontent.title',trim($data['articleTitle']));
		    }
			if(trim($data['short_name'])!=""){
				$this->db2->set('productmenucontent.short_name',trim($data['short_name']));
			}
			if(trim($data['articleDesc']) != ""){
			   $this->db2->set('productmenucontent.description',trim($data['articleDesc']));
		    }
			if(trim($data['articleStatus']) != ""){
			   $this->db2->set('productmenucontent.status',trim($data['articleStatus']));
		    }
			$this->db2->where('id', $data['articleID']);
		    $rs=$this->db2->update('productmenucontent');
			if($rs){
				return true;
			}else{
				return false;
   ...
   function deleteArticleData($articleID) {
	   $CI = &get_instance();
		...
		if($articleID != ""){
		  $this->db2->where('productmenucontent.id',$articleID);
		  $this->db2->delete('productmenucontent');
...
function getTotalProducts(){
  ...
  function getMenuContentDetails($id='',$per_page,$page,$searchdata)
  {
		$CI = &get_instance();
		//setting the second parameter to TRUE (Boolean) the function will return the database object.
		$this->db2 = $CI->load->database('ADMINDB', TRUE);
		...
		if($searchdata!="")
		{
			if($id>0)
			{
				...
				$this->db2->where('productmenucontent.menu_id',$id);
				$this->db2->where('productmenucontent.title',$searchdata);
			}
		}
		else
		{
		 ...
		 $this->db2->limit($per_page,$page); // pagging added by SB on 27/04/2015
		$query=$this->db2->get();
  ...
  public function insertDataToTable($tbl='',$data=array()){
	...
	$CI = &get_instance();
	   ...
	   $this->db2->select('productmenucontent.*');
	   $this->db2->from('productmenucontent');
	   $this->db2->where('productmenucontent.parent_article_id',0);
	   if($menuId>0){
		   $this->db2->where('productmenucontent.menu_id',$menuId); 
	   }
	   $query=$this->db2->get();
	   return $query->result_array(); 
  }
  ...
  function insertArticleDetails($data=array()){
   ...
   function insertArticle($data=array()){
	  ...
	  $CI = &get_instance();
	   ...
	   if(!empty($data)){
		   if(trim($data['menuTypeID'])!=""){
				$this->db2->set('productmenucontent.menu_id',trim($data['menuTypeID']));
			}
			if(trim($data['articleTypeID'])!=""){
				$this->db2->set('productmenucontent.parent_article_id',trim($data['articleTypeID']));
			}
			if(trim($data['articleTitle'])!=""){
				$this->db2->set('productmenucontent.title',trim($data['articleTitle']));
			}
			...
			if(trim($data['fashionPosition'])!=""){
				$this->db2->set('productmenucontent.position',trim($data['fashionPosition']));
			}
			$this->db2->insert('productmenucontent');
  ...
  function CT_insertArticleDetails($data=array()){
   ...
   public function getupdateArticleList($menuId,$article_id)
   {
	   $CI = &get_instance();
		//setting the second parameter to TRUE (Boolean) the function will return the database object.
		$this->db2 = $CI->load->database('ADMINDB', TRUE);
	   $query =$this->db2->query("select * from productmenucontent where id = '$article_id' AND menu_id = '$menuId'")->result_array();
	   return $query;
   }
