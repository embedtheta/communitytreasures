   ...
   function convertCurrency($from, $to){
	...
	public function getCommissionDetail($userId){
		...
		$sql = "SELECT
					...
					LEFT JOIN userinfo UINFO
					ON UINFO.uID = MCAD.paymentFromUserId
					LEFT JOIN gbe_payment GPAY
					ON GPAY.id=MCAD.paymentId
					WHERE MCAD.paymentToUserId =".$userId."
					 ORDER BY MCAD.createdDate DESC
					";
		$query = $this->db->query($sql);
	...
	public function getProductCommissionDetail($userId){
		...
		$sql = "SELECT
					...
					LEFT JOIN afro_order_detail AOD
					ON AOD.orderPaymentID = MCAD.paymentId
					LEFT JOIN afro_payment APAY
					ON APAY.id=MCAD.paymentId
					WHERE MCAD.paymentFor = 'Product Purchase' 
					AND MCAD.paymentToUserId =".$userId."
					ORDER BY MCAD.createdDate DESC
					";
	...
	public function getWithdrawalLoanRequest($userId=0,$transactionType){
		// get detail of user site specific changed by SB on 02/11/2015
		$whereCon = "";
		if($userId>0){
	...
	function updateCurrentAccountDetail($data=array()){
	 ...
	 function addNotification($data=array()){
		 if(!empty($data)){
			...
			if(trim($data['forWebsite'])!=""){
				$this->db->set('notification.forWebsite',trim($data['forWebsite']));
			}
	 ...
	 function getNotification($userType,$userLevel){
		 ...
		 $this->db->where('notification.forWebsite',$this->forWebsite);
	...
	function getTopUpDetail($userid,$transactionId=0){
		$whereCon = " ";
		if($transactionId!=0){
			$whereCon = " AND gp.transaction_Id=".$transactionId;
		}
		$sql = "select gp.id,DATE_FORMAT(gp.responseEnvelope_timestamp,'%d %b %Y') AS topUpDate,gp.status,FORMAT(gp.gross_total, 2) as gross_total,gp.transaction_Id,mca.commAmt,mca.myCurrency 
						FROM gbe_payment gp
						LEFT JOIN my_current_account mca
						...
						$whereCon." ORDER BY gp.id DESC
					";
		$query = $this->db->query($sql);
		return $query->result_array();
	}
	//get POPUP detail added by SB on 11/09/2015
	function getPopUpDetail($userid,$transactionId=0){
		$whereCon = " ";
		if($transactionId!=0){
			$whereCon = " AND gp.transaction_Id=".$transactionId;
		}
	...
	function getPopUpDetailChildUser($transactionId=0){
		$whereCon = " ";
		if($transactionId!=0){
			$whereCon = " AND gp.transaction_Id=".$transactionId;
		}
		$sql = "select gp.transaction_Id,DATE_FORMAT(gp.responseEnvelope_timestamp,'%d %b %Y') AS createdDate,gp.gross_total,gp.parent_commossion,unf.firstName,unf.lastName,unf.userLevel,unf.currency 
						FROM gbe_payment gp
						LEFT JOIN userinfo unf
						ON gp.user_id =unf.uID
						WHERE gp.payment_for=2 ".$whereCon;
		$query = $this->db->query($sql);
		return $query->result_array();
	}
	function getPopUpDetailProduct($transactionId=""){
		$whereCon = " ";
		if($transactionId!=""){
			$whereCon = " AND ap.transaction_Id='".$transactionId."'";
		}
		$sql = "SELECT	ap.id,
						DATE_FORMAT(ap.order_date,'%d %b %Y') AS payment_Date,ap.payment_gross,ap.transaction_Id,aod.id,aod.productName,aod.productPrice,aod.productQuantity,aod.subTotal,MCAD.paymentAmt
						FROM afro_payment ap
						LEFT JOIN afro_order_detail aod
						ON aod.orderPaymentID=ap.id
						LEFT JOIN my_current_account_details MCAD
						ON MCAD.paymentId=aod.orderPaymentID
						WHERE MCAD.fromWebsite='AFRO' ".$whereCon;
		$query = $this->db->query($sql);
		return $query->result_array();
	}
