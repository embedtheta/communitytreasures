    ...
    public function index() { 
        ...
        if (!trim($this->session->userdata('userId'))) {
            redirect(base_url() . 'gateway/', 'refresh');	
			    ...
			    $userinfoArray = $this->gatewaymodel->getUserInfoRave($userId);
			    $idlink = base64_encode($userinfoArray[0]['referarID'].'#'.$userinfoArray[0]['uID']);
		...
		$idlink = base64_encode($userinfoArray[0]['referarID'].'#'.$userinfoArray[0]['uID']);
		$viewData["idlink"] = $idlink;
		$viewData['userPaymentStatus'] = $this->getPaymentStatusOfUser($userinfoArray[0]['emailID']);
		$viewData["userInfo"] = $userinfoArray;
