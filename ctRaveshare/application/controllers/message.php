...
class Message extends CI_Controller {
    ...
    public function index() {
        ...
        $resendEmail_url = $this->config->item('resendEmail_url'); 

		$idlink = base64_encode($viewData["userInfo"][0]['referarID'].'#'.$viewData["userInfo"][0]['uID']);

		$signUpLink = 'commonSignUpForFounder/'.$idlink;

		$viewData["signUpPaymentLink"] = $resendEmail_url.'signup/'.$signUpLink;
		...
		############## Subhendu: Here I am checking the existance of the user in CT website 06-12-2016 ####################

		$emailID = $viewData["userInfo"][0]['emailID'];
		$viewData['userPaymentStatus'] = $this->getPaymentStatusOfUser($emailID);

		

		############## END of existance checking ################################################################
  ...
  <p style="font-size:14px;">Message:     '.$details[0]->message.'</p>
	...
	public function myRefCycleView($userId){
		...
		if(count($result) > 0){
			foreach($result as $rd){
				...
				$retData[$rd->referarID][$rd->uID]['cuLName'] = $rd->cuLName;
				$retData[$rd->referarID][$rd->uID]['cuEmail'] = $rd->cuEmail;
				$retData[$rd->referarID][$rd->uID]['currentPaymentstatus'] = $this->getPaymentStatusOfUser($rd->cuEmail);
				$retData[$rd->referarID][$rd->uID]['cuPhone'] = $rd->cuPhone;
				$retData[$rd->referarID][$rd->uID]['created_date'] = $rd->created_date;
				$retData[$rd->referarID][$rd->uID]['afrooPaymentStatus'] = $rd->afrooPaymentStatus; // added by SB on 22/08/2016
				$retData[$rd->referarID][$rd->uID]['userType'] = $rd->userType; // added by SB on 22/08/2016
	...
	public function reSendMail(){
			...
			if($resendStatus){  
				...
				$val = array("success" => "yes","message"=> $resendStatus );
...
################# Developed By Subhendu Das on Dated 23-11-2016 #############
     ...
     public function reSendInvitationMail(){

		    $resendEmail_url = $this->config->item('resendEmail_url'); 

		    $referrerId  	= trim($_POST["referrerId"]);
           ...
           /* echo "<pre>";
			...
			$resendStatus =  $this->resendEmailToVipUser($referrerId,$userId,$sign_up_user_type,$uName,$uEmail);
				...
				if($resendStatus){
					...
					$tblType = 'rave_resendInvitation';

					$typeData =array();

					$typeData['userId'] = $userId;

					$typeData['email'] = $uEmail;

					$typeData['raveType'] = $sign_up_user_type;// my Type
...
public function reSendInvitationMailToSponsor($resendlink ){
    $resendlink =$_GET["link"];


	$resendlink = base64_decode($resendlink);
	$resendlinkArray = explode("#", $resendlink);
	        $referrerId  	= trim($resendlinkArray[0]);
	//print_r($resendlinkArray); exit;
			$uType  	= trim($resendlinkArray[1]);

			$userId   	= trim($resendlinkArray[2]);

			$uName    	=  trim($resendlinkArray[3]);

			$uEmail    	=  trim($resendlinkArray[4]);
			...
			$resendStatus =  $this->resendEmailToVipUser($referrerId,$userId,$sign_up_user_type,$uName,$uEmail);
				...
				if($resendStatus){
                    ...
                    echo "Mail has been successfully submitted."; exit;
					

				}
				else{
                     echo "Unable to send the email."; exit;
...
<h4>SO MOVE VERY FAST!</h4><br />
								  ...
								  <tr>
									...
									<td align="left" valign="top" style="border-top:0; border-bottom:0;">

									<br />

<a href="'.$resendEmail_url.'signup/'.$signUpLink.'" target="_blank" style="background: #1bb58f none repeat scroll 0 0; border-radius: 5px; clear: both; color: #fff; font-size: 16px; margin: 10px 0; padding: 10px; text-decoration: none;">Click this link</a>
								  ...
								  <tr><td style="border-top:0; border-bottom:0;"><p>Enter This Username & Password</p></td></tr>

								  <tr><td style="border-top:0; border-bottom:0;">

									  <table width="80%" border="0" cellspacing="0" cellpadding="0" align="center" style="margin:0 auto;">
        ...
        //$this->_subject = "VIP Position For Launch of CT.";
             ...
             $this->_subject = "ENTER HERE Community Treasures";
		//echo $this->_to_email."===".$this->_subject."++++".$this->_message; exit;
