										 ...
										 else
										{
											 ...
											 // echo "==".$comAddedToTopMem;
												 if($comAddedToTopMem){
													 ...
													 // add per
													 $perAddedToTopMem = $this->gatewaymodel->addCommissionToTopmembernRef($commMember,$per,0);
													 if($perAddedToTopMem){
															// add Top Member share detail to log transaction
														 $instLogOpp 				= array();
														 $instLogOpp['note'] 		= "per from Admin to ParentUser-".$commMember;
														 $instLogOpp['action'] 		= "-";
														 $instLogOpp['amount'] 		= $per;
														 $instLogOpp['tranDate'] 	= date('Y-m-d');
										 ...
										 else
										{
											 if($commMember!=0){
													...
													if($userlevel==1){
														...
														$activeCom=9.38;
														$founderCom=6.25;
														$commToInviter		= 1.88; // commission to inviter of level 3
														// commission to opportunity  & Karma
														$opportunity = 9.38;
														$per       = 3.13;
														$karma       = 9.38;
										 ...
										 else
										{
											 if($commMember!=0){
													...
													if($userlevel==1){
														...
														$founderCom=3.13;
														$commToInviter		= 0.94; // commission to inviter of level 2
														// commission to opportunity ,per & Karma
														$opportunity = 2.34;
														$per       = 1.56;
														$karma       = 1.59;
														$level3Entrance = 9.38; // entrance level 3
													}
													else if($userlevel==3){
														$activeCom=9.38;
														$founderCom=6.25;
														...
														$per       = 3.13;
														$karma       = 9.38;
														$level3Entrance = 25;  // entrance level 4
													}
													else if($userlevel==4){
														$activeCom=18.75;
														$founderCom=9.38;
														$commToInviter		= 3.75; // commission to inviter of level 4
														// commission to opportunity ,cooperative & Karma
														$opportunity = 37.50;
														$coop       = 15.63;
														$karma       = 62.50;
														$level3Entrance = 53.13;  // entrance level 5
													}
													if($commMemberDetail['raveType']=="Active"){
											 ...
											 // echo "==".$comAddedToTopMem;
												 if($comAddedToTopMem){
													 ...
													 // add coop
													 $coopAddedToTopMem = $this->gatewaymodel->addCommissionToTopmembernRef($commMember,$coop,0);
													 if($coopAddedToTopMem){
															// add Top Member share detail to log transaction
														 $instLogcoop 				= array();
														 $instLogcoop['note'] 		= "coop from Admin to ParentUser-".$commMember;
														 $instLogcoop['action'] 		= "-";
														 $instLogcoop['amount'] 		= $coop;
														 $instLogcoop['tranDate'] 	= date('Y-m-d');
														 
														 $insertLogcoop =$this->gatewaymodel->tranRecordInter($instLogcoop);
  ...
  public function moveUpTonextLevel5(){
		...
		if(count($eleMoveUpUser)>0){
			...
			foreach($eleMoveUpUser as $eleMoveUpUserDetail){
				...
				$raveType = $eleMoveUpUserDetail['raveType'];//$this->session->userdata('raveType');
				if($userId!= ""){
					$levelUpPrice		= 1000; //second level entry fee $1000
										 ...
										 else
										{
											 if($commMember!=0){
													...
													if($userlevel==1){
														...
														$activeCom=37.50;
														$founderCom=15.63;
														$commToInviter		= 3.75; // commission to inviter of level 5
														// commission to opportunity ,Vaccation & Karma
														$opportunity = 140.63;
														$vacation       = 53.13;
														$karma       = 15.63;
											 ...
											 // echo "==".$comAddedToTopMem;
												 if($comAddedToTopMem){
													 ...
													 $vaccationAddedToTopMem = $this->gatewaymodel->addCommissionToTopmembernRef($commMember,$vacation,0);
													 if($vaccationAddedToTopMem){
															// add Top Member share detail to log transaction
														 $instLogentvacc 				= array();
														 $instLogentvacc['note'] 		= "Vaccation from Admin to ParentUser-".$commMember;
														 $instLogentvacc['action'] 		= "-";
														 $instLogentvacc['amount'] 		= $vacation;// entrance level 5
														 $instLogentvacc['tranDate'] 	= date('Y-m-d');
														 
														 $insertLogvacc =$this->gatewaymodel->tranRecordInter($instLogentvacc);
