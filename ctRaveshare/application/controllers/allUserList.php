...
class AllUserList extends CI_Controller {
    ...
    public function index() { //echo $this->session->userdata('userId')."++++".$this->session->userdata('userType');
		   ...
		   if($this->session->userdata('userType')=="ADMIN" || $this->session->userdata('emailId')=="nichole_monteiro@yahoo.co.uk"){
	        }
	        else
	        {
...
foreach ($viewData['userListDetail'] as $key=>$line)
  {
      ...
      $parent_user_array = $this->gatewaymodel->getParentUserId($line['emailID']);
  	  $referrerDetails = $this->getReferalIdDetails($parent_user_array[0]['referarID']);
	  ...
	  $data[$key]['referralSum'] =$line['referralSum'];
  	  ...
  	  $data[$key]['UserStatus'] = $this->checkUserStatusByEmailId($line['emailID']);
...
fputcsv($file, array("UID", 'First Name', 'Last Name', 'User Name', 'Email ID',
	                 ...
	                 'referralSum', 'My Income', 'Active Commission', 'Inviters Commission', 'Founders Commission','Contingent Commission','Payment Status','Status'));
...
public function getReferalIdDetails($referarID){
       ...
       // echo "<br/><b>checkPaymentStatusByEmailId:</b>".$this->db->last_query()."<br/>";
	    ...
	    $admin_db->select('orderID,userID,emailID,userEmailId');
        $admin_db->from('crypto_payments');
        $admin_db->where('txConfirmed','1');
        $admin_db->where('userEmailId',$EmailID);
        $query2 = $admin_db->get();
        $result2 = $query2->result_array();
		...
		public function checkUserStatusByEmailId($EmailID){
        $admin_db = $this->load->database('ADMINDB', TRUE);
        ...
        if(isset($result2[0]['emailID']) && strstr($result2[0]['emailID'], '@')){
        	return 'LIVE';
        }
        else{
           return 'Pending';
 ...
 public function sponsorList() { //echo $this->session->userdata('userId')."++++".$this->session->userdata('userType');
		...
		$userDetailsArray = $this->gatewaymodel->getUserDetailsArray($emailID);
        $viewData['userListDetail'] = $userDetailsArray;
        $viewData['userListDetail'][0]['userPaymentStatus'] = $this->gatewaymodel->getPaymentStatusOfUser($emailID);
      
        foreach ($userDetailsArray as $key => $value) {
          $referarID = $value['referarID'];
          $sponsorArray = array();
          $recursiveResult = $this->gatewaymodel->getImmediateSponsorDetails($sponsorArray,$referarID);
          $viewData['userListDetail'][$key]['sponsorList'] = $recursiveResult;
			/*echo "<pre>";
		...
		$this->load->view('raveshare/sponsorList',$viewData);
