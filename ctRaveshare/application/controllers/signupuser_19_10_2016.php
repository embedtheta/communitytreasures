		...
		if ($this->input->post('submit')) {
			...
			if ($this->form_validation->run() != FALSE) {
				...
				if($raveSignUpId){
					...
					if($raveFreeEntry){
						//createCurrentAccount($signupId);// 0 balance account created added by SB 
						$this->sendEmailToVipUserFounder($instData, 'Welcome To Community Treasures User Group');
						//$this->sendEmailToAdmin($viewData, 'New Signup of COMMUNITY ' . $this->sign_up_user_type);
						$viewData = $this->resetValueAfterSuccess();
						$viewData['msgType'] = 1; //1=su;2=fa
						$viewData['msg'] = 'You have successfully completed the signup process.<br> Please check your mail for further instructions';
		...
		if ($this->input->post('submit')) {
			...
			if ($this->form_validation->run() != FALSE) {
				...
				if($raveSignUpId){
					...
					if($raveFreeEntry){
						//createCurrentAccount($signupId);// 0 balance account created added by SB on 07/08/2015
						$this->sendEmailToVipUserIndustryLeader($instData, 'Welcome To Community Treasures User Group');
						//$this->sendEmailToAdmin($viewData, 'New Signup of COMMUNITY ' . $this->sign_up_user_type);
						$viewData = $this->resetValueAfterSuccess();
						$viewData['msgType'] = 1; //1=su;2=fa
						$viewData['msg'] = 'You have successfully completed the signup process.<br> Please check your mail for further instructions';
	...
	public function reEnterUser(){
		...
		if(count($userList)>0){
			...
			foreach($userList as $userDetail){
				...
				if($userDetail['referarID']==0){
					$instData['referarID'] 				= 0;
				
				}
				else{
					$instData['referarID'] 			= $userDetail['referarID'];
				}
				...
				if($userDetail['afrooPaymentStatus']=='1'){
					...
					$tblType 				= 'user_raveshare_type';
					$typeData 				=  array();
					$typeData['user_id'] 	=  $userDetail['uID'];
					...
					$levelId = 1;
					$cycle   = 1;
					$raveFreeEntry = $this->userLevelOneFreeEntry($userDetail['uID'],$levelId,$cycle);
					
					// now delete the user from dummy rave type table 
					$tbl = 'dummy_rave_type';
					$where['user_id'] = $userDetail['uID'];
				...
				$where1['uID'] = $userDetail['uID'];
				$this->common_model->deleteDataFromTable($tbluser,$where1);
				echo "<br>successfully re-enter ".$i."====".$userDetail['uID']." user";
