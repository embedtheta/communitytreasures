...
END as payment_currency');
	...
	function isMemberExistRave($UID){
	...
	function chkUserAuthRave($emailID,$pass){
		...
		if( $emailID !="" && $pass !="" ){
			...
			//$this->db->where('rave_userinfo.forWebsite',$this->forWebsite);// 15/10/2015 ujjwal sana commented
...
/// all +1
	...
	function moveUserPosition($userId){
	...
	function moveSelectedUserPosition($currentCycle, $level,$noPositionChangeUser=array()){
		...
		if(sizeof($noPositionChangeUser)>0){
			...
			echo "<br/>moveSelectedUserPosition:".$sql ="UPDATE rave_share SET userPosition=userPosition+1 
			...
			echo "<br/>moveSelectedUserPosition:".$sql ="UPDATE rave_share SET userPosition=userPosition+1 WHERE `userCycle`=".$currentCycle." AND `level`=".$level." AND `status`='1' AND userPosition<1089";
	...
	/* function getCommMember($level){

		 $sql ="SELECT RS.userPosition,RS.user_id,RS.status,RS.counter,URT.raveType 

					FROM rave_share RS 

					LEFT JOIN user_raveshare_type URT ON RS.user_id=URT.user_id

					WHERE RS.level=".$level." AND RS.status=1 AND RS.userPosition IN (1,11,21,31,41,51,61,71,81,91,101,111,121,131,141.151)"; 
	...
	function checkCapExist($level,$date){

		$numberOfMember = 0;

		$sql ="SELECT numOfMember FROM raveCapChk WHERE `level`=".$level." AND joinDate='".$date."'";
	...
	/* function getMyReferer($parentUserId){

		$refId = 0;

		

		$sql ="SELECT referarID
		...
		if($res[0]['referarID']!=1000){

			$refId =$res[0]['referarID'];
	...
	function getMyRaveAccType($userId){

		

		$sql ="SELECT raveType FROM user_raveshare_type WHERE user_id=".$userId;

		$query = $this->db->query($sql);

		$res = $query->result_array(); 		

		$myRaveType =$res[0]['raveType'];
	...
	function myCurrPosition($userId,$level){

		$myRavePosition=0;

		$sql ="SELECT userPosition FROM rave_share WHERE `level`=".$level." AND user_id=".$userId;
		...
		if($res[0]['userPosition']!=""){

			$myRavePosition =$res[0]['userPosition'];

		}

		

		return $myRavePosition; 		

	}

	

	function getUserNotPurchaseCatalog(){

		$sql ="SELECT RU.uID ,URT.raveType

				FROM rave_userinfo RU

				LEFT JOIN user_raveshare_type URT

				ON RU.uID=URT.user_id 

				WHERE RU.afrooPaymentStatus='0' AND RU.forWebsite=2 ORDER BY RU.uID asc";
	...
	function getRefererCommDetail($userId,$level){

		$sql ="SELECT rave_share.user_id,rave_share.level,rave_share.status,rave_share.counter,rave_userinfo.emailID 

					FROM rave_share 

					LEFT JOIN rave_userinfo 

					ON rave_share.user_id=rave_userinfo.uID

					WHERE rave_share.level=".$level." AND rave_share.user_id IN( SELECT uID FROM rave_userinfo WHERE referarID=$userId)";
	...
	function getMoveUpUserForCycle($currPosition,$level){
	...
	function referralAmountArray($userId){
		$sql ="SELECT SUM(invtAmt)  refTotal,SUM(rave_referralDetail.refCommCount) refCommCount

						FROM rave_referralDetail

						WHERE rave_referralDetail.userId=".$userId;
	...
	function chkRefDataExist($myReferrer,$userId,$level,$cycle){
		...
		echo "<br>chkRefDataExist:".$sql = "SELECT id 
	...
	function getMyReferralsNew($userId,$level){

		$myreferralTotal =0;

		$sql ="SELECT count(id)  myreferralTotal
	...
	public function checkingCtSignupPaymentExist($uID){

		$this->db->select('RU.uID,RU.afrooPaymentStatus');

		$this->db->from('rave_userinfo RU');
	...
	/*===========================================Added By Debayan on 15.07.2016=============================================*/
	...
	public function getRaveActiveUser(){

		$sql= "SELECT firstName,emailID,afrooPaymentStatus 

						FROM rave_userinfo						

						WHERE afrooPaymentStatus= 1 
	...
	public function getNextCycleUser(){

		$sql= "SELECT uID 

						FROM rave_NextCycleUser						
	...
	function chkUserAuthRaveForPass($emailID){

		if( $emailID !=""){

			$this->db->select('rave_userinfo.firstName,rave_userinfo.lastName,rave_userinfo.emailID,rave_userinfo.password,rave_userinfo.afrooPaymentStatus');

			$this->db->from('rave_userinfo');

			$this->db->where('rave_userinfo.emailID',$emailID);			

			//$this->db->where('rave_userinfo.forWebsite',$this->forWebsite);
	...
	// userReenter related function 

	function getDummyUser($noUser){

            

		$sql = "SELECT dummy_rave_user.*,dummy_rave_type.ravetype 

						FROM dummy_rave_user 

						LEFT JOIN dummy_rave_type 

						ON dummy_rave_user.id=dummy_rave_type.user_id

						ORDER BY dummy_rave_user.id ASC
	...
	##### created by subhendu on 09-11-2016 #############
...
AND `rave_userinfo`.`emailID` ='".$emailID."' AND `rave_share`.status = '1'";
	...
	function getLatestCycle(){
		...
		$sql = "SELECT Max( userCycle ) AS curCycle, id, userPosition, level
FROM `rave_share`
WHERE STATUS = '1'";		
	...
	function insertAllUsersToDummyTable(){
 
		$sql = "SELECT rave_userinfo.*,user_raveshare_type.ravetype as rvtype 

						FROM rave_userinfo 

						LEFT JOIN user_raveshare_type 

						ON rave_userinfo.uID=user_raveshare_type.user_id

						ORDER BY rave_userinfo.uID ASC";		
	    ...
	    foreach ($resultset as $key => $value) {
	    	# code...
	    	$chk_sql = "SELECT COUNT(*) from dummy_rave_user WHERE emailID='".$value['emailID']."'";		
            ...
            if($count==0){
            	  $status = $value['status'];
            	if($value['afrooPaymentStatus']=="1"){
                  $status = '1';
            	}
	    	$sql = "INSERT INTO `dummy_rave_user`(`uID`, `referarID`, `raveType`, `firstName`,
	    	 `lastName`, `userName`, `emailID`, `password`, `phone`, `gender`, `occupation`, `address`,
	    	  `city`, `state`, `country`, `zip`, `currency`, `webUrl`, `profile`, `paypalAC`, `switchOnPayment`, `mssLevelUpPayment`, `tenDollerPayment`, `afrooPaymentStatus`, `skypeID`, `email_send_status`, `userType`, `userLevel`, `status`, `created_date`, `confirmStatus`, `confirmedOn`, `activatedOn`, `facebookLink`, `myBlogger`, `twitterLink`, `youTubeUrl`, `forWebsite`) 
	    	VALUES (
	    	'".$value['uID']."',
	    	'".$value['referarID']."',
	    	'".$value['rvtype']."',
	    	'".mysql_real_escape_string($value['firstName'])."',
	    	'".mysql_real_escape_string($value['lastName'])."',
	    	'".mysql_real_escape_string($value['userName'])."',
	    	'".$value['emailID']."',
	    	'".$value['password']."',
	    	'".$value['phone']."',
	    	'".$value['gender']."',
	    	'".mysql_real_escape_string($value['occupation'])."',
	    	'".$value['address']."',
	    	'".$value['city']."',
	    	'".$value['state']."',
	    	'".$value['country']."',
	    	...
	    	'".$value['profile']."',
	    	'".$value['paypalAC']."',
	    	'".$value['switchOnPayment']."',
	    	'".$value['mssLevelUpPayment']."',
	    	'".$value['tenDollerPayment']."',
	    	'".$value['afrooPaymentStatus']."',
	    	'".$value['skypeID']."',
           ...
           $newReferarID = $this->getNewReferrerId($value['referarID']);
           $this->updateUserRowData($uID,$newReferarID,$value['userName']);
           if(strlen($value['rvtype'])>2){
           $drt_sql = "INSERT INTO `dummy_rave_type`(`user_id`, `raveType`) 
           VALUES 
           (
           '".$uID."',
	       '".$value['rvtype']."'
           ...
           $this->db->query($drt_sql);
       }//end empty checking of user type
            } //end count
            else{

            	echo "<h1>Duplicate record with ID: ".$value['uID']."==>".$value['emailID']."</h1>";
      ...
      exit;
		...
		############ Developed by subhendu on dated 17-11-2016 #########
		function getNewReferrerId($referarID){
         if($referarID>0){
		$chk_sql = "SELECT emailID FROM `rave_userinfo` WHERE uID='".$referarID."'";		
		$query = $this->db->query($chk_sql);
		$result = $query->row_array();
		$emailID = $result['emailID'];

		$chk_sql = "SELECT id from dummy_rave_user WHERE emailID='".$emailID."'";		

		    $query = $this->db->query($chk_sql);
		    $result = $query->row_array();
            return $newReferrerId = $result['id'];
		...
		function updateUserRowData($uID,$newReferarID,$userName){
			$usernameArray = explode(".",$userName);
			$total_element = count($usernameArray);
			$usernameArray[$total_element-1]=$uID;
			$userName = implode(".", $usernameArray);
			
        $sql ="UPDATE dummy_rave_user SET newReferarID=".$newReferarID." ,
	...
	public function getAllNextCycleUser($userPosition){
		...
		echo "<br>getAllNextCycleUser:".$sql = "SELECT RNC.uID,RNC.firstName,RNC.lastName,RNC.userName,RNC.emailID,RNC.password,RNC.phone,RNC.afrooPaymentStatus,RNC.skypeID,RNC.userType,RNC.forWebsite,RS.userCycle,RS.userPosition,RS.id as raveShareId,URT.raveType 
					FROM rave_userinfo RNC 
					LEFT JOIN rave_share RS 
					ON RNC.uID=RS.user_id
					LEFT JOIN user_raveshare_type URT
					ON RNC.uID=URT.user_id
					WHERE RS.status='1' AND RNC.status IN('1','2') AND RNC.afrooPaymentStatus='1'
	...
	function isUserMailExist($emailID=""){
	...
	public function getSubUserLists($current_user_array,$userId){
	...
	public function getUserDetailsArray($emailId){
       ...
       // echo "<br>".$this->db->last_query();
		...
		public function getParentUserId($emailId){
...
######### sreela create dummy user(24-2-17)################
		
		public function getLiveUnpaidUser($noUser)
		{
			
			$query=$this->db->query("select * from dummyaccount where referalId = 1 and flag = 0 LIMIT 0 ,1")->result_array();
			
			return $query;
		}
		public function updateDataFromTable($tbl='',$where){
		$data=array(
				'flag'=>1
		);
        if(trim($tbl)!='' && !empty($tbl) && sizeof($where)>0){
			$this->db->where('id',$where);
		    $this->db->update('dummyaccount',$data);
