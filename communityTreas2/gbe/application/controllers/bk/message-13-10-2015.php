	...
	public function addMassUser(){
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() != FALSE) {
				if($this->userType == 'ADMIN'){
					$viewData["parentID"] = $this->userId; //block by SB on 08/10/2015
					//$userLevel = trim($this->input->post('userLevel')); //block by SB on 08/10/2015
					$userLevel = 0; //added by SB on 08/10/2015
				}
				else{
				...
				//$this->insertMssDetails($userLevel,$signupId,$message);// block by SB on 08/10/2015
				$this->insertMssDetailsNew($userLevel,$signupId,$message,$viewData["parentID"]); // block by SB on 08/10/2015
  ...
  <p style="font-size:14px;">Message:     '.$details[0]->message.'</p>
	...
	public function selectParentVolunteer($city){
		...
		// check level 1 volunteer  having child count less than 12 take 1
			$minUserChildCount = $this->message_model->volChildLessThanTwelve(1,$city);// doing rana 
			
			if(count($minUserChildCount)>0){
				$i=0;
				 foreach($minUserChildCount as $minUserChildCounts){
					 
					 if($minUserChildCounts['total']<12){
						$i++; 
						$volData['selectedParentId'] =$minUserChildCounts['to_user_id'];
						if($i = 1)
						{
							break;
						}
				...
				$volData['volLevel']= 2;
					...
					$minUserChildCount2 = $this->message_model->volChildLessThanTwelve(2,$city);// doing me
				
					if(count($minUserChildCount2)>0){
							$i=0;
							 foreach($minUserChildCount2 as $minUserChildCounts2){
								 
								 if($minUserChildCounts2['total']<12){
									$i++; 
									$volData['selectedParentId'] =$minUserChildCounts2['to_user_id'];
									if($i = 1)
									{
										break;
									}
							...
							$volData['volLevel']= 3;
								...
								if(count($minUserChildCount3)>0){
									foreach($minUserChildCount3 as $minUserChildCounts3){
									 
									 if($minUserChildCounts3['total']<12){
										$i++; 
										$volData['selectedParentId'] =$minUserChildCounts3['to_user_id'];
										if($i = 1)
										{
											break;
										}
