...
class Gbe_payment extends CI_Controller {
    ...
    public function catalog($catalogId = 0) {
		...
		// add to commission table start added by SB on 17/07/2015			
			...
			$status 			= 1;			
			$userDetail = $this->addCommissionToUserAccount($this->session->userdata('referarId'),$userId,$sendDataArray['parent_commossion'],$sendDataArray['payment_currency'],$invoice,$paymentFor,$status);
	...
	function addCommissionToUserAccount($referarId,$userId,$paymentAmt,$payment_currency,$invoice,$paymentFor,$status){
			$commData = array();
