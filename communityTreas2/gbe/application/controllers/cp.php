  ...
  <p style="font-size:14px;">Password:'.$details[0]->cupwd.'</p>
    ...
    public function viewAdsList($id){
       ...
       // $viewData["list"] = $this->common_model->fetchDataFromTable($tbl,$where,$selectedData);
		...
		$tempList = $this->common_model->fetch_countries($config["per_page"], $page, $viewData["selectedAds"], $id);
   ...
   /*  echo "<pre>";
	...
	/*public function viewsimAdsList(){
        ...
        $viewData['navMenu'] = $this->data_model->getMenusList();
		$config = array();
        $config["base_url"] = base_url() . "cp/viewsimAdsList";
        $config["total_rows"] = $this->common_model->record_simcount();
        $config["per_page"] = 10;
        ...
        $tempList = array();
        $tempList = $this->common_model->fetch_simcountries($config["per_page"], $page);
		 foreach($tempList as $key=>$val){
            $tempList[$key]->categoryName = $this->common_model->getCategoryTitle($val->categoryId);
        }
        ...
        $this->load->view('adminTemplates/ads/view_simulator_ads.php', $viewData);


    }*/
    
	###########################  Modified 03-01-2017 Ad Post ################
	
	 public function resizeimg($image, $path, $logo, $height = '', $weight = '') {
        $this->load->library('SimpleImage');
        $this->SimpleImage = new SimpleImage();
        $this->SimpleImage->load('adminuploads/adpost/' . $image);
        ...
        if($_FILES["ad_image"]["name"] != ""){
            ...
            if (($_FILES["ad_image"]["size"] < 20000000) && in_array($extension, $allowedExts)) {
                ...
                if ($_FILES["ad_image"]["error"] > 0) {
					...
					$this->resizeimg($_FILES["ad_image"]["name"],base_url()."adminuploads/adpost".DIRECTORY_SEPARATOR,'','165','158');
   ...
   /* public function resize($image_data) {
    ...
    public function deleteAds($id = 0){
        ...
        $tbl = "admin_ads";
			...
			$unlinkPath = "pdf_upload";
			$pdfD = $this->common_model->fetchAdContent($tbl, $where, $unlinkPath);
			$selectedData = "ad_image";
			$imageD = $this->common_model->fetchAdContent($tbl, $where, $selectedData);
			$delPath = $this->common_model->imageUnlinkPath().'adminuploads/adpost/'.$imageD[0]->ad_image;
			$delPDFPath = $this->common_model->imageUnlinkPath().'adminuploads/pdf_upload/'.$pdfD[0]->pdf_upload;
			if(file_exists($delPath)){
				unlink($delPath);
			}
			if(file_exists($delPDFPath)){
				unlink($delPDFPath);
			}
			//$this->imageUnlink($unlinkPath);
			unset($unlinkPath);
	...
	/*public function deleteSimAds($id = 0){
        $where = array("id" => trim($id));
        $tbl = "admin_ads";
	...
	###########################  START 02-01-2017 Manage Ad Content ################
	...
	###########################  END 03-01-2017 Manage Ad Content ################
	###########################  START 04-01-2017 MANAGE CATEGORY BY RUMA ################
