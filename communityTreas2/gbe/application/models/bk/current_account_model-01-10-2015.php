...
class current_account_model extends CI_Model{
   ...
   function convertCurrency($from, $to){
	...
	function getPopUpDetailChildUser($transactionId=0){
		...
		$sql = "select DATE_FORMAT(gp.responseEnvelope_timestamp,'%d %b %Y') AS createdDate,gp.gross_total,gp.parent_commossion,unf.firstName,unf.lastName,unf.userLevel,unf.currency 
						FROM gbe_payment gp
	...
	function getPopUpDetailProduct($transactionId=""){
		...
		$sql = "SELECT	ap.id,
						DATE_FORMAT(ap.order_date,'%d %b %Y') AS payment_Date,ap.payment_gross,aod.id,aod.productName,aod.productPrice,aod.productQuantity,aod.subTotal,MCAD.paymentAmt
