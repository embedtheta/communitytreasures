...
class current_account_model extends CI_Model{
   ...
   function convertCurrency($from, $to){
	...
	//get TOPUP detail added by ujjwal sana on 7/10/2015
	...
	function updateCurrentAccountDetail($data=array()){
	 ...
	 function addNotification($data=array()){
		 if(!empty($data)){
			...
			if(trim($data['forWebsite'])!=""){
    			$this->db->set('notification.forWebsite',trim($data['forWebsite']));//4/11/2015 added by ujjwal sana 
  			 }
	 ...
	 function getNotification($userType,$userLevel){
		 ...
		 $this->db->where('notification.notBy',0);
		 $this->db->where('notification.forWebsite',$this->forWebsite);//2/11/2015 added by ujjwal sana
		 $this->db->order_by("notification.id DESC");
         $query = $this->db->get();
         //echo $this->db->last_query();    
         return $query->result_array();
