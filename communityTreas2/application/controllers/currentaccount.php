...
class Currentaccount extends CI_Controller {
			public $forWebsite;//4/11/2015 added by us
    function __construct() {
		...
		$this->forWebsite = trim($this->session->userdata('forWebsite'));//4/11/2015 added by us
    }
	...
	public function myAccount(){ 
		...
		//print_r($viewData);exit;//2/11/2015
	...
	public function editPaypalAcID(){
		$paypalAc = trim($_POST["paypalAc"]);		
		if ($paypalAc != "") 
		{
            $tbl = 'userinfo';
            $where = array("uID"=>$this->session->userdata("userId"));
            $data = array("paypalAC"=>$paypalAc) ;
            $update = $this->common_model->updateDataToTable($tbl, $where, $data );
            if($update){
              $val = array();
			  $val=array("success" => "yes","message"=>'PaypalAc Sucessfully Updated');
            }
			else
			{
				$val=array("success" => "No","message"=>'Unable To Update PaypalAc');
			}
		}
		$output = json_encode($val);
		echo $output;
	...
	public function notificationSend(){
		...
		if($notTitle!="" && $notMessage!=""){
			...
			$data['forWebsite']  = $this->forWebsite;//4/11/2015 added by us
	...
	public function showPopUpDetail(){
		...
		if($transactionId!="" && $transactionType!=""){
			...
			if($transactionType=="TopUp"){
				...
				$popUpData = $this->current_account_model->getPopUpDetail($userid,$transactionId);
				...
				$popUpData = $this->current_account_model->getPopUpDetailChildUser($transactionId);
				...
				$popUpData = $this->current_account_model->getPopUpDetailProduct($transactionId);
				//print_r($popUpData);exit;
				$tblTitle		= '<h4>Product Commission Selling Details</h4>';
				$tblHeader		='<table width="100%" cellspacing="0" cellpadding="0" border="0">
									  <tbody>
										<tr>
										  <th>Transaction Id</th>
			...
			if(count($popUpData)>0){
				...
				foreach($popUpData as $popUpdetail){
						...
						if($transactionType=="TopUp"){
							...
							<td>".$currSymbol." ".number_format((float)$popUpdetail['gross_total'], 2, '.', '')."</td>
							<td>".$popUpdetail['createdDate']."</td>
							<td>".$currSymbol." ".number_format((float)$popUpdetail['parent_commossion'], 2, '.', '')."</td>
							";
						}
						else if($transactionType=="Product"){
							
							$tblData .= "<tr><td>".$popUpdetail['transaction_Id']."</td>
							<td>".$popUpdetail['productName']."</td>
							<td>".number_format((float)$popUpdetail['productPrice'], 2, '.', '')."</td>
							<td>".$popUpdetail['productQuantity']."</td>
							<td>".$popUpdetail['payment_Date']."</td>
							";
						}
				...
				if($i>1){
					$tblData .= "<tr><td>Payment Gross:</td><td>".$popUpData[0]['payment_gross']."</td><td colspan='2'>My Commission:</td><td>".$popUpData[0]['paymentAmt']."</td></tr>";
				}
				$tblData .= '</tbody></table>';
