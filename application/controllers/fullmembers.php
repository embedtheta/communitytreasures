...
class Fullmembers extends CI_Controller {
    ...
    function __construct() {
		...
		//$this->_admin_email = "ujjwal.sana92@gmail.com";//21/09/2015 ujjwal sana this should be change
    ...
    public function index() {
		...
		$this->ckEditor($width);
        if($this->referarId == 0) {
            $viewData["parentInfo"] = $this->gatewaymodel->getUserInfo(1000);//this should be change 1000
        }else{
		...
		$viewData["allPost"] = $this->fullmembers_model->getAllPost($this->userId);
		...
		$viewData['totalMembersUnderMe'] = $this->fullmembers_model->getTotalMembersUnderMe($this->userId);// blocked by SB on 11/12/2015
		$viewData['totalMembersUnderMeNew'] = $this->common_model->getTotalMembersUnderMeNew($this->userId);// count of member logged in to site under me added by SB on 11/12/2015
		...
		$viewData['searchEvBred'] = $this->fullmembers_model->getEventSearchBredCamp($viewData['eventSearchData']);
		//print_r($viewData);exit;
		$viewData['levelName'] = 2;// added by SB on 08/03/2016
       	$this->load->view('fullmembers/index',$viewData);
		unset($viewData);
    }
	//8/10/2015 ujjwal sana added  start
	...
	public function setCalenderYearMonth($year,$month){
		...
		if(is_numeric($month)){
			$this->session->set_flashdata('month',intval($month));
		}
...
//9/10/2015 end
	...
	public function addPost(){
		...
		if($this->input->post('submitPost')):
			...
			$imgData = $this->blogImageUpload(1);//18/09/2015 added ujjwal sana
			if($imgData['status'] == 3){
	...
	public function copyPostAdd(){
		...
		if($this->input->post('submitPost')):
			...
			$imgData = $this->blogImageUpload(2);//18/09/2015 added ujjwal sana
			if($imgData['status'] == 3){
				$insertData['post_image'] = $imgData['post_image'];	
			}
			/*$insertData['post_category_id'] = trim($this->input->post('post_type_id'));
	...
	public function blogImageUpload($tempImg = 1){
		...
		else if (trim($this->input->post('tempImage'))!= "") {
			$path = $this->common_model->imageUnlinkPath()."useruploads/blogs/others/";
			$file = $path.trim($this->input->post('tempImage'));
			if(@getimagesize($file)){
				
				$newfile = 'copy-'.time().trim($this->input->post('tempImage'));
				$newfilepath = $path.$newfile;
				if (copy($file, $newfilepath)) {
					$retData['post_image'] = $newfile;
					$retData['status'] = 3;
				}
				else{
					$retData['status'] = 2;
	...
	public function downloadBrochure($fileName){
		set_time_limit(0);
		$path =$this->config->item('gbe_image_upload_path')."adminuploads/brochureVcards/brochure/".$fileName; // change the path to fit your websites document structure
		//print_r($path);exit;
		$name = $fileName;
		$mimType = " ";
	...
	public function downloadVcards($fileName){
		set_time_limit(0);
		$path =$this->config->item('gbe_image_upload_path')."adminuploads/brochureVcards/vcards/".$fileName; // change the path to fit your websites document structure
		//print_r($path);exit;
		$name = $fileName;
		$mimType = " ";
		$this->download($path,$name,$mimType);
	...
	public function sendAgrreEmail(){
        ...
        $this->_subject = "Agree user in CT";
