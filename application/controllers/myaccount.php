...
class Myaccount extends CI_Controller {
    ...
    public function index() { 
        ...
        if (!trim($this->session->userdata('userId'))) {
            ...
            redirect(base_url() . 'gateway/', 'refresh');	
		       ...
		       $userId = $this->session->userdata('userId');
				...
				$viewData["userdata"] = $mainuserinfoArray;
			  	/*if($mainuserinfoArray[0]['account_type_ap'] == '1') {
		...
		if($userinfoArray[0]['confirmStatus']==1){
          ...
          $viewData["msginfo"] = "Please contact with site Admin.Account activation is still Pending.";
			...
			#################### START RIGHT PANEL USER POSITION #########
		...
		*/
		...
		$viewData['raveActiveUser'] = $this->gatewaymodel->getRaveAllUser(1);// active member count
		 /* echo "<pre>";
			...
			//$viewData["referred_user_list"]=$this->common_model->fetch_ref_user_list($this->session->userdata('userId'));
			$viewData["referred_user_list"]=$this->common_model->fetch_popup_status($this->session->userdata('userId'));
			$viewData["referred_user_count"]=$this->common_model->count_ref_user_list($this->session->userdata('userId'));
			$userId = $this->session->userdata('userId');
			$viewData["user_id"] = $userId;
			$viewData["userdata"]  = $this->gatewaymodel->getUserInfo($userId);
				//print_r($viewData["categoryList"]);exit;
		//print_r($viewData["referred_user_count"]);exit;
	...
	public function update_flag()
	{
		...
		$auth_code= $this->input->post('ac');
		$data1= $this->common_model->send_referrer_email($id,$auth_code);
		
		$data2= $this->common_model->update_referral_user($id,$position,$auth_code);
