...
class Gbe_payment extends CI_Controller {
	...
	public function returnUrl(){
		redirect(base_url() . 'dashboard');//13/10/2015 ujjwal sana added
	}
    ...
    public function display(){
	 ...
	 public function switchOn($switchOnLevel=0){
		...
		else if($switchOnLevel==2){
			$grossTotal = 60;
			$adminComm = 20;
			$parentComm = 40;
			$paymentFor 		= 'L2 Switchon';
		}
		...
		$status 			= 1;	
		//   add Payment to Parent 	7/10/2015 us	
		$userDetail = $this->addCommissionToAdminParentAccount($parentInfo[0]['uID'],$userId,$grossTotal,$parentComm,$parentInfo[0]['currency'],$paymentId,$paymentFor,$status);
		...
		if($adminDetail){
			...
			// update user level in user table
				$updateUserLevel				=$this->gatewaymodel->updateUserLevel($this->session->userdata('userId'));
			// Update switchOn payment in User table
				if($switchOnLevel==1)
				{
						$tbl = "userinfo";
			...
			// redirect to Full member Page
				...
				$parentData['userName']				=	$viewData["userInfo"][0]['firstName']."".$viewData["userInfo"][0]['lastName'];
				...
				$this->sendEmailSwitchOnPaymentToParent($parentData);
				...
				$adminData['userName']				=	$viewData["userInfo"][0]['firstName']."".$viewData["userInfo"][0]['lastName'];
				$adminData['transaction_Id']		=	$invoiceDetail[0]->transaction_Id;
				$this->sendEmailSwitchOnPaymentToAdmin($adminData);
				
				// mail to self
				$myData = array();
				$myData['email'] 					=	$viewData["userInfo"][0]['emailID'];
				$myData['grossAmt']					=	$grossTotal;
				$myData['paymentCurrency']			=	$paymentCurrency;
				$myData['userName']					=	$viewData["userInfo"][0]['firstName']."".$viewData["userInfo"][0]['lastName'];
				$myData['transaction_Id']			=	$invoiceDetail[0]->transaction_Id;
				$this->sendEmailSwitchOnPaymentToUser($myData);
				
				$viewData["balance"]		=	$viewData["accountInfo"][0]['commAmt'];
				if($switchOnLevel==1)
				{
					$viewData["actionPage"] 	=   "fullmembers";
				}
				else if($switchOnLevel==2){
					$viewData["actionPage"] 	=   "diversity";
				}
				$viewData["paypalmsg"]		=	"SwitchOn Payment is Successful,  Your account balance is now ".$viewData["currSymbol"]." ".$viewData["balance"];
	 ...
	 function sendEmailSwitchOnPaymentToUser($data = array()) {
       
        ob_start();
        $this->load->view('email_templates/sendEmailSwitchPaypalPaymentToUser', $data);
        $content = ob_get_clean();
        $subject = 'SwitchOn Invoice Details';
        $to = $data['email'];
        $this->send_mail_raw($to, $subject, $content);
        return true;
    }
	 function sendEmailSwitchOnPaymentToParent($data = array()) {
        
        ob_start();
        $this->load->view('email_templates/sendEmailSwitchPaypalPaymentToParent', $data);
        $content = ob_get_clean();
        $subject = 'New SwitchOn Invoice Details';
        $to = $data['email'];
        $this->send_mail_raw($to, $subject, $content);
        return true;
    }

    function sendEmailSwitchOnPaymentToAdmin($data = array()) {
	...
	*/
    ...
    private function insertPaymentSwitch($userId = 0,$productId = 0,$resArray = array(),$sendDataArray = array()) {
	 ...
	 private function insertVirtualPaymentSwitch($userId = 0,$adminComm,$parentComm,$grossTotal,$payment_currency) {
        ...
        $data['user_id'] = $userId;        
        $data['ip'] = $_SERVER['REMOTE_ADDR'];
        $data['payment_for'] = 2; //1=afroweb catalog,2=level 1 switch on
        $data['level'] = 1;
	...
	function addCommissionToAdminParentAccount($referarId,$userId,$paymentGrossAmt,$paymentAmt,$payment_currency,$invoice,$paymentFor,$status){
			...
			//$commData["remark"]				= '';
			$commData["status"] 			= $status;
			$commData["fromWebsite"]		= "RAVE";// added by SB on 13/10/2015
			//$userDetail = addCommisionToAccount($commData);
	...
	public function returnUrlTopup(){
			...
			$viewData["balance"]		=	$viewData["accountInfo"][0]['commAmt'];
			$viewData["actionPage"] 				="currentaccount/myAccount";
	...
	public function updatePaymentTopUp(){
        ...
        if (trim($_POST['tracking_id']) != '' && trim($_POST['pay_key']) != '') {
            ...
            if (trim($_POST['status']) == 'COMPLETED') {
				...
				unset($updateComm);
				//get user detail check if user status is 2 (blocked) set it 1 
				$userDetail = $this->gatewaymodel->getUserInfo($invoiceData[0]->user_id);
				if($userDetail[0]['status']==2){
					$this->gatewaymodel->changeUserStatus($invoiceData[0]->user_id,1);// status 1 of permission changed
				}
            }
