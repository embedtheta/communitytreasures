...
class Payment extends CI_Controller {
    ...
    public function index( ) {
		...
		$tmpvalue 	= 	$this->uri->segment(3);
		$pos_hash	=	strpos($tmpvalue,"_");
		$tmpprice	=	substr($tmpvalue,0,$pos_hash);
		$tmpdis		=	substr($tmpvalue,$pos_hash);
		
		$price		=	base64_decode($tmpprice);
		$discount	=	base64_decode($tmpdis);
		
		if($price =="" && $discount=="" && $userID == "")
		{
			
			$viewData['acc_price']		= '140';			
			$viewData['price']			= '140';	
		}
		else if($price =="" && $discount=="" && $userID != "")
		{
			$viewData['acc_price']		= '140';			
			$viewData['price']			= '';
		}
		else if(($price !="" || $discount!="") && $userID == "")
		{
			...
			$viewData['acc_price']		=	$price - $discount;
		...
		$viewData['user_details']	=	$this->paymentmodel->userDetailsForAPBBacc($userID);
	...
	public function chkUser() {
		...
		$recExists				= $this->paymentmodel->isMailExist($emailID);
		if($recExists) {
	...
	public function success() {
		...
		$viewData['paymentDate']		= $paymentDate;
		$viewData['email']				= $email;
		$viewData['account_type']		= $account_type;
		$viewData['ID']					= $uID;
		if(!empty($uID)){
			if($account_type == 'BB')
			{
				$this->paymentmodel->updatePaymentDataBBacc($uID);
				$this->paymentmodel->updateDetailsBBacc($uID,$amount,$category,$Industry);
			}
			else if($account_type == 'AP')
			{
				$this->paymentmodel->updatePaymentDataAPacc($uID);
				$this->paymentmodel->updateDetailsAPacc($uID,$amount,$category,$Industry);
				...
				if($account_type == 'BB')
				{
					...
					$account_type_bb 	= '1';					
					$payment_amt_AP  	= '0';
					$payment_amt_BB		= '120.00';
				}
				else if($account_type == 'AP'){
					$account_type_ap 	= '1';
					$account_type_bb 	= '0';					
					$payment_amt_AP  	= '140.00';
					$payment_amt_BB		= '0';
				}
				$insArr					= array(
										"referarID"			=> "0",
										...
										"country"			=> $country,
										"userType"			=> 'PAYING USER',
										"account_type_ap"	=> $account_type_ap,
										"account_type_bb"	=> $account_type_bb,
										"forWebsite"		=> '2',
										"created_date"		=> date("Y-m-d h:i:s")
										);//echo "<pre>";print_r($insArr);exit;
				$insID					= $this->paymentmodel->insertData($insArr, 'userinfo');
				$rlnsArr				= array(
										...
										"industry_id"			=> $Industry,
										"payment_ref"			=> $ref,
										"payment_amt_AP"		=> $payment_amt_AP,
										"payment_amt_BB"		=> $payment_amt_BB
										);
				$this->paymentmodel->insertData($rlnsArr, 'user_account_rlns');
				
			$viewData['account_type']		= '';	
		}	
		$viewData['user_id']	= $insID;
		$this->load->view('payment/success', $viewData);
		/*echo $fname=$get_val[0];
	...
	public function newBBsuccess_url(){
	
		$this->load->view('payment/newBBsuccess');	
	}
	public function signup() {		
		$viewData							= array();
		$userID								= $this->input->post('txtid');
		$userDetails						= $this->paymentmodel->getUserInfo($userID);
		$viewData['bb_activation_status']	= $userDetails[0]['bb_user_activation_status'];
		$viewData['account_type_bb']		= $userDetails[0]['account_type_bb'];			
		...
		if($this->input->post('txtsignup') == 'confirm') {			
			...
			if($editdata) {
				...
				$bbstatus 	=     	$this->input->post('txtbbstatus');
				$bb			=		$this->input->post('txtbb');
				if($bbstatus == 'N' && $bb	== '1'){
					
					redirect(base_url() . 'payment/newBBsuccess_url');	
				}
				else{
					...
					$this->session->set_userdata('forWebsite', $userDetails[0]['forWebsite']);
    ...
    public function bbRegistration(){
		$viewData					= array();
		$userID						=	trim($this->session->userdata('userId'));
		$tmpvalue 	= 	$this->uri->segment(3);
		...
		$viewData['user_details']	=	$this->paymentmodel->userDetailsForAPBBacc($userID);
		//print_r($viewData['user_details']);exit;
		$this->load->view('payment/BB_registration', $viewData);
