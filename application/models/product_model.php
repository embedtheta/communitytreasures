...
class product_model extends CI_Model{
    ...
    public function getOffer($productId = 0){
        $this->db->select('*');
        $this->db->from('product_offer');
        $this->db->where('product_offer.product_id', $productId);
        $query = $this->db->get();
        //echo $this->db->last_query();
        //print_r($query);exit;
        return $query->result();
	...
	public function getProductCategory($productTypeID){
		$data	=	array();
		$result = $this->db->query("select * from productmenucontent where id= '".$productTypeID ."'")->row();		
		if(!empty($result)){
			if($result->menu_id == '18')
			{
				$data['category'] = 'Miscellaneous';
			}
			$data['subcategory'] = $result->title;
			$parent_article_id = $result->parent_article_id;
			if($parent_article_id != '0')
			{
				$data['subcategory_id'] = $result->id;
				$data['main_category_id'] = $parent_article_id;
				$result2	=	$this->db->query("select * from productmenucontent where id= '". $parent_article_id ."'")->row();
				$data['subcategory2'] = $result2->title;
	...
	public function getCityNameByProductOffer($cityid){

		$result = $this->db->query("select * from city where id='".$cityid."'")->row();
		if(!empty($result)){
