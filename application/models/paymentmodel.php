...
class Paymentmodel extends CI_Model {
	...
	public function getIndustryList() {
        ...
        $this->db->where('ct_monetizer.status', '0');
        $query = $this->db->get();
        //echo $this->db->last_query();
        return $query->result_array();
	}
	
	public function delData($tableName = '', $conditionArr = array()) {
    ...
    public function updateData($dataArr = array(), $tableName = '', $conditionArr = array()) {
        if(count($conditionArr) > 0) {
	...
	public function updatePaymentDataBBacc($uID){
		
		if($uID !="" ){				

			$sql = "UPDATE userinfo SET account_type_bb= '1' WHERE uID=".$uID;
	...
	public function updateDetailsBBacc($uID,$amount,$category,$Industry){
		
		if($uID !="" ){				
			$query = $this->db->query("select id from user_account_rlns where user_id = '" .$uID."'")->row();

			if(!empty($query->id)){
				
				$sql = "UPDATE user_account_rlns SET payment_amt_BB = '" .$amount ."' WHERE user_id =".$uID;
	...
	public function updatePaymentDataAPacc($uID){
		
		if($uID !="" ){				

			$sql = "UPDATE userinfo SET account_type_ap= '1' WHERE uID=".$uID;
	...
	public function updateDetailsAPacc($uID,$amount,$category,$Industry){
		...
		if($uID !="" ){				
			...
			if(!empty($query->id)){
				...
				$sql = "UPDATE user_account_rlns SET payment_amt_AP = '" .$amount ."' WHERE user_id =".$uID;
...
END as payment_currency');
	...
	function getEmailParent($parentId){
		
		$sql ="SELECT emailID FROM `rave_userinfo` WHERE `uID` =".$parentId;
...
/// all +1
		...
		echo "<br/>".$sql ="SELECT userPosition,user_id FROM rave_share WHERE `level`=".$level." AND userCycle=".$cycle." AND userPosition = ".$levelPosition."  ORDER BY id asc LIMIT 1";// added by SB 16/09/2016
...
AND `rave_share`.`userPosition` =".$userPosition." AND `rave_share`.status = '1'";
	...
	public function userDetailsForAPBBacc($userId){
		$query = $this->db->query("select uf.*,(select cat_id from user_account_rlns where user_id = uf.uID)as category_id ,(select industry_id from user_account_rlns where user_id = uf.uID)as industry_id,(select payment_ref from user_account_rlns where user_id = uf.uID)as payment_ref from userinfo uf where uID ='" .$userId."'")->row();
