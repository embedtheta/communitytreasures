...
public function imageUnlinkPath() {
	...
	public function ct_dealcategory()//this portion have to be change
	{
		$sql="select * from ct_deal_day";
		//echo $sql; die;
		$query=$this->db->query($sql);
		return $query->result_array();
	}
   ...
   public function get_visited_status($user_id,$ad_id)
   {
	...
	$this->db2->where('date(watch_video_task.visited_date)',$cdate);
...
//print_r($data);exit;
   ...
   public function fetchBanner() {
	...
	{
		$data=array();
		$query['details']= $this->db->query("select popup_status,userPosition from referral_popup_user where referarID= '$user_id' and popup_status= 0")->result();
		{
			if(!empty($query['details']))
			{
				foreach($query['details'] as $qd)
				{
					$data[]= $qd->userPosition;
				}
				$query['count'] = array_count_values($data);
				 
				return $query;
			}
		}	
	}
	public function fetch_popup_status($user_id)
	{
		
		$query= $this->db->query("select popup_status from referral_popup_user where referarID =$user_id")->result();
	...
	public function Get_Referral_User(){
				...
				if(!empty($sql2))
				{
					foreach($sql2 as $ss)
					{
						...
						if(($ss->userPosition) >= 65 && ($ss->userPosition)< 320)
						{
							...
							$query=$this->db->query("select * from referral_popup_user where user_id= '$ss->uID' and entry_status='1' and userPosition= '65' and Userlevel= '2' and userCycle= '1' and email_send_status ='y'");
						...
						else if(($ss->userPosition)>= 320 && ($ss->userPosition)< 577)
						{
							...
							$query=$this->db->query("select * from referral_popup_user where user_id= '$ss->uID' and entry_status='1' and userPosition= '320' and Userlevel= '3' and userCycle= '1' and email_send_status ='y'");
							$count= $query->num_rows();
							if($count== 0)
							{
								$date = date("Y-m-d H:i:s");
						...
						else if(($ss->userPosition) >= 577 && ($ss->userPosition)< 834)
						{
							...
							$query=$this->db->query("select * from referral_popup_user where user_id= '$ss->uID' and entry_status='1' and userPosition= '577' and Userlevel= '4' and userCycle= '1' and email_send_status ='y'");
							$count= $query->num_rows();
							if($count== 0)
							{
								$date = date("Y-m-d H:i:s");
						...
						else if(($ss->userPosition) >= 834 && ($ss->userPosition)< 1089)
						{
							...
							$query=$this->db->query("select * from referral_popup_user where user_id= '$ss->uID' and entry_status='1' and userPosition= '834' and Userlevel= '5' and userCycle= '1' and email_send_status ='y'");
							$count= $query->num_rows();
							if($count==0)
							{
								$date = date("Y-m-d H:i:s");
						...
						else if(($ss->userPosition) >= 1089)
						{
							...
							$query=$this->db->query("select * from referral_popup_user where user_id= '$ss->uID' and popup_status='1' and userPosition= '834' and Userlevel= '5' and userCycle= '1' and email_send_status ='y'");
							$count= $query->num_rows();
							if($count==0)
							{
								$date = date("Y-m-d H:i:s");
		...
		$sub = 'Congratulation! You have earned money for the users You have reffered' ;
				...
				$query= $this->db->query("select rpu.*,(select lastName from rave_userinfo where emailID=rpu.emailID) as lastName,(select emailID from rave_userinfo where uID=rpu.referarID) as receiver_emailID,(select firstName from rave_userinfo where uID=rpu.referarID) as receiver_firstName from referral_popup_user as rpu where rpu.email_send_status='n'")->result();
				...
				if(!empty($query))
				{
					 ...
					 foreach ($query as $qq)
					 {
						 $a_code= $qq->referarID . $qq->user_id;
							$count= $this->count_ref_user_list($qq->referarID);
								if (array_key_exists("65",$count['count']))
								{
									
									$auth_code=  $a_code.'65';
									$body = "Hello " . $qq->receiver_firstName . ",<br><br>";
									$body .= "Congratulation! You have earned $15 for the user <b>". $qq->firstName ." ". $qq->lastName . "</b>(<b>" . $qq->emailID ."</b>) whome you have referred, is moved up from level 1 to 2. <br>";
									$body .= "Please put this authentication code<strong>". $auth_code ."</strong> in the popup box while login, as your verification. <br><br>";
									$body .= '<strong>With Regards</strong><br>'; 
									$body .= '<strong>Community Treasures</strong>';
									$to = $qq->receiver_emailID .',test2.senabi@gmail.com';

									$from = 'info@communitytreasures.co';
									$this->sendMail($to,$from,$sub,$body);
									$mail_status_update=$this->db->query("update referral_popup_user set email_send_status ='y',authentication_code= '$auth_code' where referarID= '$qq->referarID' and userPosition= '65' and user_id= '$qq->user_id'");
									
										
								}
								if (array_key_exists("320",$count['count']))
								{
									$auth_code= $a_code.'320';
									$body = "Hello " . $qq->receiver_firstName . ",<br><br>";
									$body .= "Congratulation! You have earned $30 for the user <b>". $qq->firstName ." ". $qq->lastName . "</b>(<b>" . $qq->emailID ."</b>) whome you have referred, are moved up from level 2 to 3. <br><br>";
									$body .= "Please put this authentication code<strong>". $auth_code ."</strong> in the popup box while login, as your verification. <br><br>";
									$body .= '<strong>With Regards</strong><br>';
									$body .= '<strong>Community Treasures</strong>';
									$to = $qq->receiver_emailID .',test2.senabi@gmail.com';
									...
									$from = 'info@communitytreasures.co';
									$this->sendMail($to,$from,$sub,$body);
									$mail_status_update=$this->db->query("update referral_popup_user set email_send_status ='y', authentication_code= '$auth_code' where referarID= '$qq->referarID' and userPosition= '320' ");
										
								}
								if (array_key_exists("577",$count['count']))
								{
									$auth_code= $a_code.'577';
									$body = "Hello " . $qq->receiver_firstName . ",<br><br>";
									$body .= "Congratulation! You have earned $60 for the user <b>". $qq->firstName ." ". $qq->lastName . "</b>(<b>" . $qq->emailID ."</b>) whome you have referred, are moved up from level 3 to 4. <br><br>";
									$body .= "Please put this authentication code<strong>". $auth_code ."</strong> in the popup box while login, as your verification. <br><br>";
									$body .= '<strong>With Regards</strong><br>';
									$body .= '<strong>Community Treasures</strong>';
									$to = $qq->receiver_emailID .',test2.senabi@gmail.com';
									...
									$from = 'info@communitytreasures.co';
									$this->sendMail($to,$from,$sub,$body);
									$mail_status_update=$this->db->query("update referral_popup_user set email_send_status ='y', authentication_code= '$auth_code' where referarID= '$qq->referarID' and userPosition= '577' ");
										

								}
								if (array_key_exists("834",$count['count']))
								{
									$auth_code= $a_code.'834';
									$body = "Hello " . $qq->receiver_firstName . ",<br><br>";
									$body .= "Congratulation! You have earned $90 for the user <b>". $qq->firstName ." ". $qq->lastName . "</b>(<b>" . $qq->emailID ."</b>) whome you have referred, are moved up from level 4 to 5. <br><br>";
									$body .= "Please put this authentication code<sctrong>". $auth_code ."</strong> in the popup box while login, as your verification. <br><br>";
									$body .= '<strong>With Regards</strong><br>';
									$body .= '<strong>Community Treasures</strong>';
									$to = $qq->receiver_emailID .',test2.senabi@gmail.com';
									...
									$from = 'info@communitytreasures.co';
									$this->sendMail($to,$from,$sub,$body);
									$mail_status_update=$this->db->query("update referral_popup_user set email_send_status ='y', authentication_code= '$auth_code' where referarID= '$qq->referarID' and userPosition= '834'");
	 ...
	 public function sendMail($to = '', $from = '',$sub = '', $body = '') {
        ...
        $config = array(
            'protocol' => 'smtp',
            'smtp_host' => 'mail.communitytreasures.co',
            'smtp_port' => 26,
            'smtp_user' => 'info@communitytreasures.co',
            'smtp_pass' => 'j+&WyFLK2CLJ0',
            'charset' => 'iso-8859-1',
            'mailtype' => 'html',
            'wordwrap' => TRUE
        );
        ...
        $this->email->from('info@communitytreasures.co');
        $this->email->to($to);
	...
	public function update_referral_user($id,$position,$auth_code)
	{
		$data= array('popup_status'=>'1'); 
		$this->db->where('userPosition',$position);
		$this->db->where('referarID',$id);
		$this->db->where('popup_status',0);
		$this->db->where('authentication_code',$auth_code);
		$this->db->update('referral_popup_user',$data);
		return true;
	}
	public function send_referrer_email($id,$auth_code)
	{
		$qry= $this->db->query("select rpu.*,(select firstName from rave_userinfo where uID=$id) as receiver_firstName, (select lastName from rave_userinfo where uID=$id) as receiver_lastName,(select emailID from rave_userinfo where uID=$id) as receiver_emailID from referral_popup_user rpu where referarID='$id' and authentication_code='$auth_code' and popup_status='0'");
		...
		if($count == '0')
		{
			echo 0;
				...
				$sub_admin = "Verification of user authenticity for getting referral amount";
				$body_admin = "Hello Admin,<br><br>";
				$body_admin .= $data->receiver_firstName ." ". $data->receiver_lastName . " is a authenticate user to get the referral amount for $data->firstName. <br><br>";
				
				$body_admin .= '<strong>With Regards</strong><br>';
				$body_admin .= '<strong>Community Treasures</strong>';
				$to_admin = 'admin@communitytreasures.co,test2.senabi@gmail.com';
				
				$sub = 'Congratulation! Successfully verification' ;
				$body = "Hello " . $data->receiver_firstName .",<br><br>";
				$body .=  "Congratulation! You have successfully verified your authenticity.<br><br>";
				
				$body .= '<strong>With Regards</strong><br>';
				$body .= '<strong>Community Treasures</strong>';
				$to = $data->receiver_emailID;

				$from = 'info@communitytreasures.co';
				
				$this->sendMail($to,$from,$sub,$body);
				$this->sendMail($to_admin,$from,$sub_admin,$body_admin);
				echo 1;
		}
	...
	public function ct_offer_list($catId){
		
			$this->db->select('*');
            $this->db->from('ct_upload_offers');			
	...
	public function ct_product_list($catId){
			
			$query = $this->db->query("select pd.*,(select fileName from product_files where productId = pd.productID AND isMain = 1) as product_img,(select voucher_code from product_offer where product_id = pd.productID) as voucher_code,(select voucher_amt from product_offer where product_id = pd.productID) as voucher_amt from product_details pd where pd.productTypeID = '" .$catId ."'")->result_array();
			return $query;
	...
	public function ct_cityproduct_list($catId,$cityId){
		
			if($catId!="All Categories" && $cityId!=""){
			$query = $this->db->query("select pd.*,(select fileName from product_files where productId = pd.productID AND isMain = 1) as product_img,(select voucher_code from product_offer where product_id = pd.productID) as voucher_code,(select voucher_amt from product_offer where product_id = pd.productID) as voucher_amt from product_details pd where pd.productTypeID = '" .$catId ."' AND pd.city = '" .$cityId ."'")->result_array();
			}
			else if($catId=="All Categories" && $cityId==""){
			$query = $this->db->query("select pd.*,(select fileName from product_files where productId = pd.productID AND isMain = 1) as product_img,(select voucher_code from product_offer where product_id = pd.productID) as voucher_code,(select voucher_amt from product_offer where product_id = pd.productID) as voucher_amt from product_details pd")->result_array();
			}
			else if($catId!="All Categories" && $cityId==""){
			$query = $this->db->query("select pd.*,(select fileName from product_files where productId = pd.productID AND isMain = 1) as product_img,(select voucher_code from product_offer where product_id = pd.productID) as voucher_code,(select voucher_amt from product_offer where product_id = pd.productID) as voucher_amt from product_details pd where pd.productTypeID = '" .$catId ."'")->result_array();
			}
			else{
			$query = $this->db->query("select pd.*,(select fileName from product_files where productId = pd.productID AND isMain = 1) as product_img,(select voucher_code from product_offer where product_id = pd.productID) as voucher_code,(select voucher_amt from product_offer where product_id = pd.productID) as voucher_amt from product_details pd where pd.city = '" .$cityId ."'")->result_array();
			}
	...
	public function ct_dealproduct_list(){
		$query = $this->db->query("select pd.*,(select fileName from product_files where productId = pd.productID AND isMain = 1) as product_img,(select voucher_code from product_offer where product_id = pd.productID) as voucher_code,(select voucher_amt from product_offer where product_id = pd.productID) as voucher_amt from product_details pd where pd.deal_flag='1' order by pd.productID DESC")->result_array();
		return $query;
	}
		
	
	public function ct_product_list_2subcat($catId){
		$pro_list = array();
		$query_cat = $this->db->query("select id from productmenucontent where parent_article_id = '" .$catId ."'")->result();
		if(!empty($query_cat))
		{
			foreach($query_cat as $cat){
				
				$query = $this->db->query("select pd.*,(select fileName from product_files where productId = pd.productID AND isMain = 1) as product_img,(select voucher_code from product_offer where product_id = pd.productID) as voucher_code,(select voucher_amt from product_offer where product_id = pd.productID) as voucher_amt from product_details pd where pd.productTypeID = '" .$cat->id ."'")->result_array();
				$pro_list[] = $query;
			}
		...
		public function ct_cityproduct_list_2subcat($catId){
		$pro_list = array();
		if($catId!="")
		{
		$query = $this->db->query("select pd.*,(select fileName from product_files where productId = pd.productID AND isMain = 1) as product_img,(select voucher_code from product_offer where product_id = pd.productID) as voucher_code,(select voucher_amt from product_offer where product_id = pd.productID) as voucher_amt from product_details pd where pd.productTypeID = '" .$catId."'")->result_array();	
	...
	public function getProductDetailsById($productID){
            ...
            $this->db->join('userinfo', 'product_details.addedBy = userinfo.uID');	
            $this->db->join('product_offer', 'product_details.productID = product_offer.product_id');	
	...
	public function fetch_admin_catalog_config() {
	    $CI = &get_instance();
		//setting the second parameter to TRUE (Boolean) the function will return the database object.
		$this->db2 = $CI->load->database('ADMINDB', TRUE);
        $this->db2->where('id','1');
        $query = $this->db2->get("admin_catalog_config");
